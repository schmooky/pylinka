...
- *Set-like* outputs (`output.setVelocity`, `output.writePosition`, `output.writeColor`, `output.writeScale`, `output.writeRotation`, `output.initLife`, `output.spawnPosition`, `output.initVelocity`, `output.initTexIndex`, `output.initRotation`) are **single-writer** — a second writer is a compile error.
...
**Generators** (`gen.*`): `random` (stable), `randomRange` (stable), `randomVec2` (stable), `frameRandom` (per-frame), `curveOverLife` (structural ease), `colorOverLife`, `scaleOverLife`, `spin` (v1 2026-08-27: angular VELOCITY in rad/s, integrated over `p.age` — the term that makes a sprite turn, as opposed to `writeRotation`, which only ever set a fixed angle), `rotationOverLife` (v1 2026-08-27: eased sweep between two angles), `noise` (impact **medium**), `curl` (M2, impact **high**).
**Math** (`math.*`): `add`, `sub`, `mul`, `div` (guarded), `mad`, `mix`, `clamp`, `min`, `max`, `sin`, `cos`, `abs`, `radians` (v1 2026-08-27: degrees to radians — every angle port is radians, artists type degrees), `length`, `normalize` (guarded), `rotate2d`, `splat`, `swizzle`, `makeVec2/4`, `component`. — `expression` is **M2** (scalar-only, whitelisted fns, parsed to AST and compiled — never string-spliced).
**Fields** (`field.*`): `gravity`, `directional` (strength+angle — the wind), `radial` (attract/repel), `drag` (exponential, §13.6), `vortex` (promoted to v1 2026-07-11: swirl around `emitterPos + center`, tangential `strength` + inward `pull`, linear falloff over `radius`, 0 = global; impact **medium**), `turbulence` (v1 2026-07-11: curl of animated value noise via central differences — divergence-free swirls; `strength`/`scale`/`speed`; impact **medium**), `obstacle` (v1 2026-07-22: a body moving THROUGH the field — radial push out of a disc with a `softness` falloff, tangential `swirl`, and `carry`, an acceleration toward the body's own `velocity` that produces the bow wave and wake; `center`/`velocity` are ABSOLUTE world like `radial`, not emitter-relative like `vortex`, so they can be driven by live vec2 knobs — that is the cursor / flying-object path; impact **low**), `curlField` (M2, impact **high**), `drawnVectorField` (M2, texture-sampled, impact **high** on T1).
**Spawn shapes** (`shape.*`, feed `output.spawnPosition`, emitter-relative): `point`, `circle`/`torus`, `rectangle`, `polygonalChain`, `burstRing`, `drawnArea` (M2, SDF). `circle` is the RING (radius exactly) and `torus` is the band between an inner and an outer radius, so a filled disc is a torus from 0 — the pair is why `circle` does not fill. `burstRing` lays its particles out evenly by position in the spawn batch rather than sampling an angle.
**Outputs** (`output.*`): `spawnPosition` (required, single), `initVelocity`, `initLife` (required, single), `addForce` (accumulating), `drag` (accumulating), `setVelocity` (single, excludes addForce), `writePosition` (escape hatch), `writeColor`, `writeAlpha` (post-color lane mask), `writeScale`, `writeRotation`, `initRotation` (v1 2026-08-27: birth angle, single-writer, init phase — without it every particle spawns at 0 and a burst spins in lockstep), `initTexIndex`, `killIf` (bool, additive OR), `killIfOutOfRect` (reference engine's `deathZone`), `reflectInRect` (reference `collide`; velocity-flip only — superseded by the `collide*` family below, kept for old documents). **Solid geometry** (v1 2026-07-22, accumulating, emitted into the POST-INTEGRATION lane so they see the step that actually crossed the surface — each resolves the penetration first, then reflects the normal component with `restitution`/`friction`): `collidePlane` (`point` + `normal`: floor, wall, slope), `collideRect` (structural `mode: inside|outside` — a box that contains, or a solid crate that ejects along the axis of least penetration), `collideCircle` (structural `mode: outside|inside`, plus the disc's own `velocity` so a moving wall kicks what it hits). **End-of-life kill is automatic scaffolding — there is no `killIfAgeOverLife` node.**
**Sub-emitter trigger** (v1 2026-08-28): `output.deathBurst` carries a structural `on: death | birth`. Both are one-frame edges on the same parent slot (alive→dead, dead→alive), read from the same buffers and costing the same; `birth` is what puts a light flash on the frame a spark appears. Compile-time constant, so a graph that never asks for it emits byte-identical shaders.
**Atlas playback** (v1 2026-08-28): `AtlasPlay` is `loop` (cycle at `fps`), `once` (stretch the strip across the particle's LIFE — `fps` is ignored, the sequence finishes as the particle dies) and `hold` (play once at `fps`, then stay on the last frame). `hold` was added because `once` looked like a broken `fps`; the two pre-existing modes keep their exact shader expressions so no authored effect shifts a frame.
